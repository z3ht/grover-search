...
void apply_x_all(std::vector<Complex>& psi, int n) {
    for (int q = 0; q < n; ++q) apply_pauli_x_on_qubit(psi, q);
}
...
int main() {
    ...
    int M = 0;
    for (int i = 0; i < N; ++i) if (is_marked(i)) ++M;
    ...
    gcfg.r_bits = n / 2;
    gcfg.cell   = 28;
    gcfg.marked_index = target;
    gcfg.fill_prob = true;
    gcfg.hue_by_phase = false;
    gcfg.two_tone_sign = true;
    gcfg.gamma = 0.5;
    ...
    acfg.marked_index = target;
    acfg.draw_all     = true;
    acfg.draw_mean    = true;
    acfg.draw_reflection = false;
    acfg.draw_theta_arc  = true;
    ...
    struct Frame {
        ...
        std::string title_grid;
        std::string title_arrows;
        std::string title_bars;
        bool draw_reflection = false;
        int iter_index = 0;
    };

    std::vector<Frame> frames;
    frames.reserve(1 + 3*k);

    frames.push_back(Frame{
            psi, {},
            "Init: uniform (grid of mini-arrows)",
            "Init: uniform (Argand arrows)",
            "Init: probabilities",
            false, 0
    });
    ...
    for (int iter = 1; iter <= k; ++iter) {
        std::vector<Complex> before_oracle = cur;
        apply_oracle_phase_flip(cur, is_marked);
        frames.push_back(Frame{
                cur, before_oracle,
        ...
        frames.push_back(Frame{
                ...
                "Diffusion (preview): magenta = 2*mean - prev marked",
                "Diffusion (preview) — iter " + std::to_string(iter) + " (probs)",
                true, iter
        });
    ...
    auto render = [&](int i){
        ...
        quantum_viz::show_grid_arrows(f.psi, n, gcfg, f.title_grid.c_str());
        acfg.draw_reflection = f.draw_reflection;
        quantum_viz::show_arrows(f.psi, n, acfg, f.title_arrows.c_str(),
                                 f.draw_reflection ? &f.prev_for_reflection : nullptr);
        quantum_viz::show_prob_bars(f.psi, n, pcfg, f.title_bars.c_str());
        if (f.iter_index > 0) {
